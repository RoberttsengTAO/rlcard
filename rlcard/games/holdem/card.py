...
class holdemCard(object):
    ...
    suit_dict = dict(zip(info['suit'], [16, 8, 4, 2]))
    rank_dict_inverse = dict(zip(list(range(len(info['number']))), info['number']))
    suit_dict_inverse = dict(zip([16, 8, 4, 2], info['suit']))
    ### set Card type's value
    def __init__(self, suit_str, number_str):
        self.suit_str = suit_str
        self.number_str = number_str
        self.suit = self.suit_dict[suit_str]
        self.rank = self.rank_dict[number_str]+2
    ...
    def __str__(self):
        ...
        """
        return self.suit_str+self.number_str
    def to_id(self):
    ...
    def from_id(cls, card_id):
      ...
      if rank == 1:
          rank += 13
      number_str = rank_dict_inverse[rank-2]
      suit_str = suit_dict_inverse[suit]
      return cls(suit_str, number_str)
  
class computer_power:
    # Compute Card
    # Card type code
    HIGHCARD = 0
    ONEPAIR = 1 << 8
  ...
  #       straight flash of rank 7 => 10000000 0111 0000
    ...
    def __search_straight(self, cards):
        ...
        card_rank = [x.rank for x in cards]
        rank_1 = -1
        rank_2 = -1 
        straight_check = lambda acc, i: acc & (bit_memo >> (r+i) & 1) == 1
        for r in range(2, 15):
          if reduce(straight_check, range(5), True): rank_1 = r
        if all([min_seq in card_rank for min_seq in [14, 2, 3, 4, 5] ]):
            rank_2 = 1
        rank = max(rank_1, rank_2)
        return rank
    ...
    def __search_flash(self, cards):
        ...
        for suit, group_obj in groupby(sorted(cards, key=fetch_suit), key=fetch_suit):
            g = list(group_obj)
            if len(g) >= 5:
                interest = [sub_hole for sub_hole in hole if sub_hole in g]
                if interest:
                    max_rank_card = max(interest, key=fetch_rank)
                else:
    ...
    def __mask_hole_low_rank(self, bit):
        mask = 15
        return bit & mask
# hole = [holdemCard("C", "Q"), holdemCard("S", "J")]
# hole1 = [holdemCard("H", "A"), holdemCard("H", "T")]
# community = [holdemCard("H", "9"), holdemCard("S", "4"), holdemCard("C", "T"), holdemCard("H", "8"), holdemCard("D", "8")]
# cards = hole + community
# cards_1 = hole1 + community
# # Card = holdemCard
# com.eval_hand(hole, community)
# com.eval_hand(hole1, community)
# com._
# # def init_deck():
# #     """
...
# # deck = init_deck()
# # hole1 = [deck[0], deck[1]]
# # community1 = [deck[2], deck[3], deck[4], deck[5], deck[6]]
# # com.
# sum([0 | 1 << card.rank for card in cards])
# keyfun = lambda memo, card: memo | 1 << card.rank
# list( map(keyfun, cards))
# bit_memo = reduce(lambda memo, card: memo | 1 << card.rank, cards, 0)
# card_rank = [x.rank for x in cards]
# rank_1 = -1
# rank_2 = -1 
